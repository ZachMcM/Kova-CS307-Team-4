...
export async function getEvent(eventId: string) : Promise<GroupEvent> {
    const {data, error} = await supabase
        .from("groupEvent")
        .select("id,groupId,title,start_date,end_date,exercise_points,rep_multiplier,weight_multiplier,goal,type")
        .eq("id", eventId);    
    if (error) {
...
export async function getGroupWorkouts(eventId: string) : Promise<GroupWorkout[]>{
    const {data, error} = await supabase
        .from("eventWorkout")
...
export function getTotalPoints(workouts: GroupWorkout[], presets: string) : Map<string, number> {
    const points = new Map<string, number>();
    for (const workout of workouts) {
        points.set(workout.id, 
...
function getCompeteEvent(data: 
    {id: any;
        ...
        exercise_points: any;
        rep_multiplier: any;
        weight_multiplier: any}) : CompeteView {
    return {
        id: data.id,
        groupId: data.groupId,
        title: data.title,
